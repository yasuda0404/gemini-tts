...
・モデル: "gemini-2.5-flash-preview-tts"（単一話者TTS対応）
...
def save_wave(filename: Path, pcm_bytes: bytes, channels: int = 1, rate: int = 24000, sample_width: int = 2) -> None:
...
def build_tts_prompt(item: Dict[str, Any]) -> str:
    """
    instruction と lang, text を使って TTS 用の自然言語プロンプトを作る。
...
def synthesize_items(items: List[Dict[str, Any]], model_name: str = "gemini-2.5-flash-preview-tts") -> None:
    """
    ...
    for item in items:
        ...
        try:
            ...
            # 失敗しても他アイテムの処理は続ける
            sys.stderr.write(f"[ERROR] id={id_}, voice={voice_name} の生成に失敗: {e}\n")
